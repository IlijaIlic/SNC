  ...
  const handleSubmit = async (values) => {
    try {
      const sigCode = await axios.get(
        `http://localhost:5555/api/SigurnosniKod/${parseInt(values.skod, 10)}`
      );
      ...
      if (fetchedSigCode.sigKod == values.skod) {
        ...
        await fetch("http://localhost:5555/poslasticari", {
          method: "POST",
          headers: {
            ...
            <div className="flex flex-row flex-wrap justify-center">
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="mt-3 text-center text-lg text-white">
                  {t(
                    "Informacije o pojedinacnim tortama mozete dodati nakon registrovanja"
                  )}
                </p>
              </div>
            </div>
