...
function ClientsOverview() {
  ...
  useEffect(() => {
    const updatedFilteredList = ListAll.filter((item) => {
      // Skip further checks for Svidjanja items
  ...
  async function DeleteMladenac() {
    try {
      ...
      // Optionally, remove from ListAll if needed
      // const updatedListAll = ListAll.filter((item) => item.ID !== ChangingObject.Mladenci_UID && item.type === 'Mladenci');
  ...
  const updateListAll = (visibleAds) => {
    ...
    console.log(combinedList); // it's updating correctly but asynchronisly
  };
    ...
    try {
      ...
      const SvidjanjaList = fetchedData.flatMap((item) => [
        ...item.LikedFotograf.map((likedItem) => ({
          ...
          ID: item.Mladenci_ID,
          // Mladenci_UID: likedItem.Mladenci_UID,
        })),
          ...
          {currentItems.map((item, index) => (
            <div className="mx-4 mb-2 flex w-full flex-col  rounded-lg bg-sncdbrown p-3 text-white hover:bg-snclblue" key={index}>
              ...
              <div>
                ...
                {item.type === 'SvidjanjaFotograf' && ( /// / Cena: 20000
                ...
                {item.type === 'SvidjanjaDekorater' && ( /// / Cena: 20000
                // Datum_Osnivanja: "2024-06-06T20:22:45.000Z"
                ...
                {item.type === 'SvidjanjaPoslasticar' && ( /// / Cena: 20000
                // Datum_Osnivanja: "2024-06-06T20:22:45.000Z"
                ...
                {item.type === 'SvidjanjaRestoran' && ( /// / Cena: 20000
                // Datum_Osnivanja: "2024-06-06T20:22:45.000Z"
