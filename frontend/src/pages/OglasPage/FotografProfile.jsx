...
function FotografProfile() {
  ...
  useEffect(() => {
    ...
    async function fetchProfileData() {
      try {
        const response = await axios.get(`http://localhost:8080/fotograf/prekouid/${auth.currentUser.uid}`);
        const fetchedData = response.data;
        console.log(fetchedData);
        setIdFotografa(fetchedData[0].FotografID);
        setNaziv(fetchedData[0].NazivAgencije);
        setOpis(fetchedData[0].Opis_Kompanije);
        ...
        setCena(fetchedData[0].Cena_Usluge);
        setCenaPoSlici(fetchedData[0].Cena_Po_Slici);
        setFreeDate(fetchedData[0].Slobodni_Termini);
        console.log(fetchedData[0]);

        const responseZakazano = await axios.get(`http://localhost:8080/zakazanifotografi/prekouid/${auth.currentUser.uid}`);
    ...
    if (change === true) {
      try {
        ...
        if (isValid) {
          ...
          await fetch(`http://localhost:8080/fotograf/prekouid/${auth.currentUser.uid}`, {
            method: 'PUT',
            headers: {
        ...
        if (selectedDate != null) {
          ...
          await fetch(`http://localhost:8080/slobodniterminifotograf/prekoid/${idFotografa}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const changing = () => (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-row">
        ...
        <input type="text" className="mx-2 rounded-md text-black shadow-md" value={cenaPoSlici} onChange={(e) => setCenaPoSlici(e.target.value)} />
