...
function OglasiPageBaker() {
  ...
  const filterOptions = (inputValue) => {
    const filteredOptions = getMostCommonOptions(FilteredBakers).filter(
  ...
  const uniqueLocations = [
    ...new Set(ListaBaker.map((baker) => baker._Lokacija)),
  ];
  ...
  const filterBakerByDate = (baker) => {
    if (startDate && endDate) {
      return (
        baker._DatumOsnivanja >= startDate && baker._DatumOsnivanja <= endDate
      );
    }
    return true;
  };
  ...
  const addBaker = async () => {
    try {
      const response = await axios.get("http://localhost:5555/poslasticari");
      ...
      const bakerList = fetchedData.map((item) => ({
        ...new Baker(
          item.poslasticar.naziv,
          item.poslasticar.email,
          item.poslasticar.cena,
  ...
  return (
    <div className="flex h-full w-full flex-col">
      ...
      <div className=" flex h-full  flex-auto">
        ...
        <div className="m-4 flex h-full  w-full flex-row flex-wrap ">
          {filteredPosts.map((baker, index) => (
            <div
              key={baker.ID}
              className="mb-8 ml-12 mr-2 h-2/6 w-3/12 min-w-96"
            >
              <OglasiCardBaker baker={baker} />
            </div>
          ))}
        </div>
      </div>
      <div className="mt-2 flex justify-center">
        {Array.from({
          length: Math.ceil(ListaBaker.length / postsPerPage),
        }).map((_, index) => (
          <button
            key={index}
...
export default OglasiPageBaker;
