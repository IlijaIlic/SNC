...
function ProfileClient() {
    ...
    async function fetchProfileData() {
      try {
        const response = await axios.get(
          `http://localhost:5555/mladenci/${auth.currentUser.uid}`
        );
        const fetchedDataMladenci = response.data;

        const responseLikedRestoran = await axios.get(
          `http://localhost:8080/getLikedRestoran/${auth.currentUser.uid}`
        );
        const restoranPromises = responseLikedRestoran.data.map((restoran) =>
          axios.get(
            `http://localhost:8080/restoran/prekoid/${restoran.Restoran_ID}`
          )
        );
        const likedRestorans = await Promise.all(restoranPromises);
        const likedRestoransData = likedRestorans.map(
          (responselikedData) => responselikedData.data
        );
        setSacuvaniRestorani(likedRestoransData.flat());

        const responseLikedDekorater = await axios.get(
          `http://localhost:8080/getLikedDekorater/${auth.currentUser.uid}`
        );
        const dekorateriPromises = responseLikedDekorater.data.map(
          (dekorater) =>
            axios.get(
              `http://localhost:8080/dekorater/prekoid/${dekorater.Dekorater_ID}`
            )
        );
        const likedDekoraters = await Promise.all(dekorateriPromises);
        const likedDekoratersData = likedDekoraters.map(
          (responselikedData) => responselikedData.data
        );
        setSacuvaniDekorateri(likedDekoratersData.flat());

        const responseLikedPoslasticar = await axios.get(
          `http://localhost:8080/getLikedPoslasticar/${auth.currentUser.uid}`
        );
        const poslasticarPromises = responseLikedPoslasticar.data.map(
          (poslasticar) =>
            axios.get(
              `http://localhost:8080/poslasticar/prekoid/${poslasticar.Poslasticar_ID}`
            )
        );
        const likedPoslasticars = await Promise.all(poslasticarPromises);
        const likedPoslasticarsData = likedPoslasticars.map(
          (responselikedData) => responselikedData.data
        );
        setSacuvaniPoslasticari(likedPoslasticarsData.flat());

        const responseLikedFotograf = await axios.get(
          `http://localhost:8080/getLikedFotograf/${auth.currentUser.uid}`
        );
        const fotografiPromises = responseLikedFotograf.data.map((fotograf) =>
          axios.get(
            `http://localhost:8080/fotograf/prekoid/${fotograf.Fotograf_ID}`
          )
        );
        const likedFotografs = await Promise.all(fotografiPromises);
        const likedFotografsData = likedFotografs.map(
          (responselikedData) => responselikedData.data
        );
        setSacuvaniFotografi(likedFotografsData.flat());

        const responseZakazano = await axios.get(
          `http://localhost:8080/zakazanomladencima/prekouid/${auth.currentUser.uid}`
        );
  ...
  useEffect(() => {
    async function fetchGuests() {
      try {
        if (idMLadenci !== 0) {
          const responseGosti = await axios.get(
            `http://localhost:8080/gosti/${idMLadenci}`
          );
  ...
  useEffect(() => {
    async function fetchPU() {
      const responseFotografZakazan = await axios.get(
        `http://localhost:8080/fotograf/prekoid/${idFotografa}`
      );
  ...
  useEffect(() => {
    async function fetchPU() {
      ...
      const responsePoslasticarZakazan = await axios.get(
        `http://localhost:8080/poslasticar/prekoid/${idPoslasticara}`
      );
  ...
  useEffect(() => {
    async function fetchPU() {
      console.log("FETCH PU");
      const responseRestoranZakazan = await axios.get(
        `http://localhost:8080/restoran/prekoid/${idRestorana}`
      );
      const dataRestoran = responseRestoranZakazan.data;
      setNazivRestorana(dataRestoran[0].Naziv);
      const responseJelovnik = await axios.get(
        `http://localhost:8080/jelovnikzakazano/${idMLadenci}`
      );
  ...
  useEffect(() => {
    async function fetchPU() {
      const responseDekoraterZakazan = await axios.get(
        `http://localhost:8080/dekorater/prekoid/${idDekoratera}`
      );
  ...
  const handleUkloniDekoratera = async (id) => {
    try {
      ...
      setSacuvaniDekorateri((prevDekorateri) =>
        prevDekorateri.filter((dekorater) => dekorater.DekoraterID !== id)
      );
    } catch (error) {
      console.error("Error deleting guest:", error);
    }
  ...
  const handleUkloniFotografa = async (id) => {
    try {
      ...
      setSacuvaniFotografi((prevFotografi) =>
        prevFotografi.filter((fotograf) => fotograf.FotografID !== id)
      );
    } catch (error) {
      console.error("Error deleting guest:", error);
    }
  ...
  const handleUkloniPoslasticara = async (id) => {
    try {
      ...
      setSacuvaniPoslasticari((prevPoslasticari) =>
        prevPoslasticari.filter(
          (poslasticar) => poslasticar.PoslasticarID !== id
        )
      );
    } catch (error) {
      console.error("Error deleting guest:", error);
  ...
  const handleUkloniRestoran = async (id) => {
    try {
      ...
      setSacuvaniRestorani((prevRestorani) =>
        prevRestorani.filter((restoran) => restoran.ID !== id)
      );
    } catch (error) {
      console.error("Error deleting guest:", error);
    }
  ...
  const overviewDiv = () => (
    <div className="flex w-full flex-col gap-2">
      <div>
        <span className="text-xs ">{t("yourName")} </span>
        <div className=""> {ime}</div>
      </div>
      <div>
        <span className="text-xs ">{t("yourSurname")} </span>
        <div className="">{prezime}</div>
      </div>
      <div>
        <span className="text-xs ">{t("partnerName")} </span>
        <div className="">{imePartnera}</div>
      </div>
      <div>
        <span className="text-xs ">{t("partnerSurname")} </span>
        <div className="">{prezimePartnera}</div>
      </div>
      <div>
      ...
      <div>
        ...
        <div className="">{brojTelefona}</div>
      </div>
    </div>
  );
  const changingDiv = () => (
  ...
  return (
    ...
    >
      <TabsList className="mx-2 mb-2 flex h-fit flex-row justify-around  rounded-lg bg-sncpink p-4">
        <Tab value={1} className="text-white hover:text-sncdbrown">
          {t("myProfile")}
        </Tab>
        <Tab value={2} className="text-white hover:text-sncdbrown">
          {t("wedding")}
        </Tab>
        <Tab value={3} className="text-white hover:text-sncdbrown">
          {t("saved")}
        </Tab>
        <Tab value={4} className="text-white hover:text-sncdbrown">
          {t("guests")}
        </Tab>
      </TabsList>
        ...
        <div className=" mx-2 flex w-full flex-col items-center justify-center rounded-md bg-gradient-to-bl from-snclpink to-sncgradientpink p-5 shadow-lg">
          ...
          <div className="flex flex-row gap-2">
            <Button
              ...
              className="rounded-md bg-sncpink p-2 text-white shadow-md hover:animate-pop hover:bg-sncgradientpink"
            >
              {t("logout")}
            </Button>
            <Button
        ...
        <div className=" flex w-full flex-col gap-3 rounded-md bg-snclpink p-5 shadow-lg">
          <div className="w-full rounded-lg bg-sncpink p-3 text-white shadow-lg">
            ...
            <div>
              ...
              {jelovnikZakazan.map((jelo) => (
                <p key={jelo.ID}>
                  {jelo.ImeJela} {jelo.Cena} {t("dinara")}
                </p>
              ))}
              <p>
                {t("Cena po osobi:")}
            ...
            <div className="flex flex-row flex-wrap items-center gap-3">
              ...
              {ocenaRestoran}
              <Button
                onClick={handleOceniRestoran}
                disabled={nazivRestorana === ""}
                className={
                  nazivRestorana === ""
                    ? "rounded-lg bg-sncdbrown p-2 text-gray-600 shadow-md "
                    : "rounded-lg bg-sncgradientpink p-2 text-white shadow-md hover:bg-snclpink "
                }
              >
                {t("Oceni Restoran")}
              </Button>
          ...
          <div className="w-full rounded-lg bg-sncpink p-3 text-white shadow-lg">
            ...
            <div className="flex flex-row flex-wrap items-center gap-3">
              ...
              {ocenaPoslasticara}
              <Button
                onClick={handleOceniPoslasticara}
                disabled={nazivPoslasticara === ""}
                className={
                  nazivPoslasticara === ""
                    ? "rounded-lg bg-sncdbrown p-2 text-gray-600 shadow-md "
        ...
        <div className=" flex w-full flex-col rounded-md bg-snclpink p-5 shadow-lg">
          ...
          <div className="flex w-full flex-row  flex-wrap items-center justify-center ">
            ...
            <div className="mx-3 w-4/5">
              {(tip === "Restoran" || tip === "Sve" || tip === "") &&
                sacuvaniRestorani.map((oglas) => (
                  <div
                    className="my-2 flex w-full flex-row justify-between rounded-md  bg-snclblue p-4 pr-10 text-xl text-white shadow-sm"
                    key={`RESTORAN${oglas.ID}`}
                  >
                    <div>
                      {t("Restoran")} - {oglas.Naziv}
                    </div>
                    <div>
                      <Tooltip title="Ukloni" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() => handleUkloniRestoran(oglas.ID)}
                        >
                          <HeartBrokenIcon />
                        </Button>
                      </Tooltip>
                      <Tooltip title="Poseti" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() => handlePosetiRestoran(oglas.ID)}
                        >
                          <ArrowForwardIcon />
                        </Button>
                      </Tooltip>
              ...
              {(tip === "Dekorater" || tip === "Sve" || tip === "") &&
                sacuvaniDekorateri.map((oglas) => (
                  <div
                    className="my-2 flex w-full flex-row justify-between rounded-md  bg-snclblue p-4 pr-10 text-xl text-white shadow-sm"
                    key={`DEKORATER${oglas.DekoraterID}`}
                  >
                    <div>
                      {t("Dekorater")} - {oglas.Ime}
                    </div>
                    <div>
                      <Tooltip title="Ukloni" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() =>
                            handleUkloniDekoratera(oglas.DekoraterID)
                          }
                        >
                          <HeartBrokenIcon />
                        </Button>
                      ...
                      <Tooltip title="Poseti" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() =>
                            handlePosetiDekoratera(oglas.DekoraterID)
                          }
                        >
                          <ArrowForwardIcon />
                        </Button>
              ...
              {(tip === "Fotograf" || tip === "Sve" || tip === "") &&
                sacuvaniFotografi.map((oglas) => (
                  <div
                    className="my-2 flex w-full flex-row justify-between rounded-md  bg-snclblue p-4 pr-10 text-xl text-white shadow-sm"
                    key={`FOTOGRAF${oglas.FotografID}`}
                  >
                    <div>
                      {t("Fotograf")} - {oglas.NazivAgencije}
                    </div>
                    <div>
                      <Tooltip title="Ukloni" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() =>
                            handleUkloniFotografa(oglas.FotografID)
                          }
                        >
                          <HeartBrokenIcon />
                        </Button>
                      ...
                      <Tooltip title="Poseti" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() =>
                            handlePosetiFotografa(oglas.FotografID)
                          }
                        >
                          <ArrowForwardIcon />
                        </Button>
              ...
              {(tip === "Poslasticar" || tip === "Sve" || tip === "") &&
                sacuvaniPosalsticari.map((oglas) => (
                  <div
                    className="my-2 flex w-full flex-row justify-between rounded-md  bg-snclblue p-4 pr-10 text-xl text-white shadow-sm"
                    key={`POSALSTICAR${oglas.PoslasticarID}`}
                  >
                    <div>
                      {t("Poslasticar")} - {oglas.Ime}
                    </div>
                    <div>
                      <Tooltip title="Ukloni" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() =>
                            handleUkloniPoslasticara(oglas.PoslasticarID)
                          }
                        >
                          <HeartBrokenIcon />
                        </Button>
                      ...
                      <Tooltip title="Poseti" arrow placement="top">
                        <Button
                          className="mx-1 rounded-lg  px-3 py-1 text-center text-xs text-white hover:text-sncdblue"
                          onClick={() =>
                            handlePosetiPoslasticara(oglas.PoslasticarID)
                          }
                        >
                          <ArrowForwardIcon />
                        </Button>
        ...
        <div className="flex flex-col rounded-md bg-snclpink p-5 shadow-lg">
          <div className=" flex w-full flex-row justify-center gap-3 ">
            <input
              ...
              className="h-8 w-32  rounded-lg bg-white p-1 text-sncdblue shadow-lg outline-none hover:ring-1 hover:ring-snclblue focus:ring-2 focus:ring-snclblue"
            />
            <input
              type="text"
              placeholder={t("guestSurname")}
              value={prezimeGosta}
            ...
            />
            <Button
              className="h-8 w-24 rounded-md bg-snclblue text-white shadow-lg hover:bg-sncdblue "
              onClick={handleDodaj}
            >
              {t("add")}
            </Button>
              ...
              <div key={gost.ID}>
                <div className="m-3 flex flex-row items-center justify-center gap-2">
                  <p>
                    ...
                    <span className="text-sncpink">{t("tableNumber")}: </span>{" "}
                    {gost.Broj_Stola}
                  </p>
                  <div>
                    <Button
                ...
                {index < fetchedGosti.length - 1 &&
                  fetchedGosti[index + 1].Broj_Stola !== gost.Broj_Stola && (
