  ...
  const handleSubmit = async (values) => {
    try {
      const sigCode = await axios.get(
        `http://localhost:5555/api/SigurnosniKod/${parseInt(values.skod, 10)}` //nije pronadjen
      );
      ...
      if (fetchedSigCode.sigKod == values.skod) {
        ...
        await fetch("http://localhost:5555/fotografi", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            Naziv: values.naziv, // Corrected property name
            Opis: values.opis, // Corrected property name
            Email: values.email,
            SigurnosniKod: values.skod,
            Cena: values.cenaUsluge, // Corrected property name
            CenaPoSlici: values.cenaPoSlici, // Corrected property name
          ...
          <div className="mt-3 flex flex-col items-center">
            <div className="flex flex-row flex-wrap justify-center">
              ...
              <div className="mx-3">
                ...
                <input
                  className={
                    errors.cenaPoSlici && touched.cenaPoSlici
                      ? inputDugmeKlasaLosa
                      : inputDugmeKlasa
                  }
                  onChange={handleChange}
                ...
                />
                {errors.cenaPoSlici && touched.cenaPoSlici && (
                  <p className="text-red">{errors.cenaPoSlici}</p>
                )}
              </div>
              <div className="mx-3">
                <p className="text-xl font-bold text-white">
