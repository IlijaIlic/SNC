...
//Slika LONGBLOB za slike               putanja
//pa posle bi bilo: Buffer.from(fs.readFileSync('./ime_slike.nastavak'))
//kada se poziva get metoda za sliku onda ide ovako nes:
//res.set("Content-disposition", 'inline; filename=' "ime_slike.nastavak");
//res.set("Content-type", "image/nastavak");
...
baza.get("/slikePoslasticar/string/:id", (req, res) => {
    ...
    //     (err, results) => {
    //         if (err) {
    //             console.error("Problem prilikom vracanja slike iz tabele SlikePoslasticar:", err);
    //             return res.status(500).send("Internal Server Error");
    //         }
...
// baza.post('/slikeDekorater/:uid', upload.array('images', 10), (req, res) => {
//     const Dekorater_UID = req.params.uid;
...
//                 conn.query('SELECT ID FROM Dekorater WHERE UID = ?', [Dekorater_UID], (err, result) => {
//                     if (err) {
//                         return reject(err);
//                     }
...
//                     const Dekorater_ID = result[0].ID;

//                     conn.query('INSERT INTO SlikeDekorater (Slike, Dekorater_UID, Dekorater_ID) VALUES (?, ?, ?)', [data, Dekorater_UID, Dekorater_ID], (err, resultSlike) => {
//                         if (err) {
//                             return reject(err);
//                         }
...
// baza.post("/slikeDekorater/:uid", upload.single("image"), async (req, res) => {
//   const { uid } = req.params;
...
//   const imeslike = rq.file.filename;

// // const imagePromises = req.files.map(file => {
// //       return new Promise((resolve, reject) => {
// //           const fileName = req.file.filename;

// //           conn.query('INSERT INTO SlikeDekorater (Slike, Dekorater_UID) VALUES (?, ?)', [fileName, uid], (err, resultSlike) => {
// //               if (err) {
// //                   return reject(err);
// //               }
// //               resolve();
...
//   try {
//     await Images.create({ imeSlike: imeslike });
//     res.json.status("ok");
//   } catch (error) {
...
//delete sa uid za Iliju ako treba
// baza.delete('/restoran/:uid', (req, res) => {
//     const { uid } = req.params;

//     const getRestoranIdQuery = `
//         SELECT ID FROM Restoran WHERE UID = ?
//     `;
...
//     const deleteJelaQuery = `
//         DELETE FROM Jela
//         WHERE Jelovnik_ID IN (SELECT ID FROM Jelovnik WHERE Restoran_ID = ?)
//     `;

//     const deletePicaQuery = `
//         DELETE FROM Pica
//         WHERE Jelovnik_ID IN (SELECT ID FROM Jelovnik WHERE Restoran_ID = ?)
//     `;
...
//         if (result.length === 0) {
//             return res.status(404).send('Nije pronadjen restoran sa datim UID-em.');
//         }

//         const restoranID = result[0].ID;

//         conn.query(deleteKorisnikQuery, [restoranID], (err, resultKorisnik) => {
//             if (err) {
//                 console.error(err);
//                 return res.status(500).send('Internal Server Error Korisnik');
//             }

//             conn.query(deleteSlobodniTerminiQuery, [restoranID], (err, resultSlobodniTermini) => {
//                 if (err) {
//                     console.error(err);
//                     return res.status(500).send('Internal Server Error Slobodni_Termini');
//                 }

//                 conn.query(deleteJelaQuery, [restoranID], (err, resultJela) => {
//                     if (err) {
//                         console.error(err);
//                         return res.status(500).send('Internal Server Error Jela');
//                     }

//                     conn.query(deletePicaQuery, [restoranID], (err, resultPica) => {
//                         if (err) {
//                             console.error(err);
//                             return res.status(500).send('Internal Server Error Pica');
//                         }

//                         conn.query(deleteJelovnikQuery, [restoranID], (err, resultJelovnik) => {
//                             if (err) {
//                                 console.error(err);
//                                 return res.status(500).send('Internal Server Error Jelovnik');
//                             }

//                             conn.query(deleteRestoranQuery, [uid], (err, resultRestoran) => {
//                                 if (err) {
//                                     console.error(err);
//                                     return res.status(500).send('Internal Server Error Restoran');
//                                 }
...
//     conn.query(deleteQuery, [uid], (err, result) => {
//         if (err) {
//             console.error(err);
//             return res.status(500).send('Internal Server Error');
//         }
...
//poslasticar put sa uid
// baza.put('/poslasticar/prekouid/:uid', (req, res) => {
//     const { uid } = req.params;
//     const { Ime, Prezime, Kratak_Opis, Email, Datum_Osnivanja, Cena_Posiljke, Lokacija, Broj_Telefona, SlobodniTermini } = req.body;
...
//         conn.query(getPoslasticarIdQuery, [uid], (err, results) => {
//             if (err) {
//                 console.error(err);
...
//             if (results.length === 0) {
//                 return conn.rollback(() => {
//                     res.status(404).send('No record found with the provided UID');
//                 });
//             }

//             const poslasticarId = results[0].ID;

//             const updatePoslasticarQuery = `
//                 UPDATE Poslasticar
//                 SET Ime = ?, Prezime = ?, Kratak_Opis = ?, Email = ?, Datum_Osnivanja = ?, Cena_Posiljke = ?, Lokacija = ?, Broj_Telefona = ?
//                 WHERE UID = ?
//             `;

//             conn.query(updatePoslasticarQuery, [Ime, Prezime, Kratak_Opis, Email, Datum_Osnivanja, Cena_Posiljke, Lokacija, Broj_Telefona, uid], (err, result) => {
//                 if (err) {
//                     console.error(err);
//                     return conn.rollback(() => {
//                         res.status(500).send('Internal Server Error');
...
//                 const deleteTerminiQuery = 'DELETE FROM Slobodni_Termini WHERE Poslasticar_ID = ?';
//                 conn.query(deleteTerminiQuery, [poslasticarId], (err) => {
//                     if (err) {
//                         console.error("Error deleting SlobodniTermini:", err);
//                         return conn.rollback(() => {
//                             res.status(500).send('Internal Server Error');
//                         });
//                     }
...
//                     if (SlobodniTermini && SlobodniTermini.length > 0) {
//                         const terminiValues = SlobodniTermini.map(termin => [termin, poslasticarId]);
//                         const insertTerminiQuery = 'INSERT INTO Slobodni_Termini (Slobodan_Termin, Poslasticar_ID) VALUES ?';
...
// baza.put('/poslasticar/:uid', (req, res) => {
//     const { uid } = req.params;
//     const { Ime, Prezime, Kratak_Opis, Email, Sifra, Datum_Osnivanja, Cena_Posiljke, Lokacija } = req.body;

//     const updateQuery = `
//         UPDATE Poslasticar
//         SET Ime = ?, Prezime = ?, Kratak_Opis = ?, Email = ?, Sifra = ?, Datum_Osnivanja = ?, Cena_Posiljke = ?, Lokacija = ?
//         WHERE UID = ?
//     `;

//     conn.query(updateQuery, [Ime, Prezime, Kratak_Opis, Email, Sifra, Datum_Osnivanja, Cena_Posiljke, Lokacija, uid], (err, result) => {
//         if (err) {
//             console.error(err);
//             return res.status(500).send('Internal Server Error');
//         }
...
//     const deleteKorisnikQuery = `
//         DELETE FROM Korisnik
//         WHERE Poslasticar_ID = (SELECT ID FROM Poslasticar WHERE UID = ?)
//     `;
...
//         conn.query(deletePoslasticarQuery, [uid], (err, result) => {
//             if (err) {
//                 console.error(err);
//                 return res.status(500).send('Internal Server Error');
//             }
...
// baza.post('/restoran', (req, res) => {
//     const { Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, UID, SlobodniTermini, Jelovnik } = req.body;
//     var restoran_id;
...
//         conn.query(
//             'INSERT INTO Restoran (Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, UID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
//             [Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, UID],
//             (err, result) => {
//                 if (err) {
//                     console.error(err);
//                     res.status(500).send('Internal Server Error');
...
//                     });
//                 }
//                 restoran_id = result.insertId;

//                 conn.query(
//                     'INSERT INTO Korisnik (UID, TIP, Restoran_ID) VALUES (?, ?, ?)',
//                     [UID, "Restoran", restoran_id],
//                     (err) => {
//                         if (err) {
//                             console.error(err);
//                             res.status(500).send('Internal Server Error');
...
//                         if (SlobodniTermini && SlobodniTermini.length > 0) {
//                             const terminiValues = SlobodniTermini.map(termin => [termin, restoran_id]);
//                             conn.query(
//                                 'INSERT INTO Slobodni_Termini (Slobodan_Termin, Restoran_ID) VALUES ?',
//                                 [terminiValues],
//                                 (err) => {
...
//                         if (Jelovnik) {
//                             const { NAZIV, Cena, TIP, Kratak_Opis, Slika, Specijalitet, Jela, Pica } = Jelovnik;

//                             conn.query(
//                                 'INSERT INTO Jelovnik (NAZIV, Cena, TIP, Kratak_Opis, Slika, Specijalitet, Restoran_ID) VALUES (?, ?, ?, ?, ?, ?, ?)',
//                                 [NAZIV, Cena, TIP, Kratak_Opis, Slika, Specijalitet, restoran_id],
//                                 (err, result) => {
//                                     if (err) {
//                                         console.error(err);
//                                         res.status(500).send('Internal Server Error');
...
//                                     const jelovnik_id = result.insertId;

//                                     if (Jela && Jela.length > 0) {
//                                         const jelaValues = Jela.map(jelo => [
//                                             jelo.Predjelo, jelo.Glavno_Jelo, jelo.Dezert, jelo.Salata, jelo.Posno, jelo.Vegetarijanska, jelo.Gramaza, jelovnik_id
//                                         ]);
//                                         conn.query(
//                                             'INSERT INTO Jela (Predjelo, Glavno_Jelo, Dezert, Salata, Posno, Vegetarijanska, Gramaza, Jelovnik_ID) VALUES ?',
//                                             [jelaValues],
//                                             (err) => {
//                                                 if (err) {
//                                                     console.error(err);
...
//                                     if (Pica && Pica.length > 0) {
//                                         const picaValues = Pica.map(pice => [
//                                             pice.Alkoholno, pice.Kolicina, jelovnik_id
//                                         ]);
//                                         conn.query(
//                                             'INSERT INTO Pica (Alkoholno, Kolicina, Jelovnik_ID) VALUES ?',
//                                             [picaValues],
//                                             (err) => {
//                                                 if (err) {
//                                                     console.error(err);
...
// baza.post('/restoran', (req, res) => {
//     const { Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, UID, SlobodniTermini } = req.body;
//     var restoran_id;
//     conn.query(
//         'INSERT INTO Restoran (Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, UID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
//         [Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, UID],
...
//         const restoraniList = Object.values(restorani);
//         res.json(restoraniList);
//     });
// });

// baza.get('/restoran', (req, res) => {
//     conn.query('SELECT * FROM Restoran', (err, result) => {
//         if (err) throw new Error(err);
//         res.json(result);
//         // console.log(result);
//         // res.end();
...
//PUT za Restoran  //Imam tvoju verziju metode sacuvao sam je ako nam zatreba - Vlastimir
// baza.put('/restoran/:id', (req, res) => {
//     const restoranID = req.params.id;
//     const {
//         Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, RestoranPraviTortu, Bend, Broj_Telefona
//     } = req.body;

//     const updateRestoranQuery = `
//         UPDATE Restoran
//         SET
//             Naziv = ?,
//             Kratak_Opis = ?,
...
//     const restoranValues = [Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, RestoranPraviTortu, Bend, Broj_Telefona, restoranID];

//     conn.query(updateRestoranQuery, restoranValues, (err, result) => {
//         if (err) {
...
//         res.status(200).send('Restaurant details updated successfully');
//     });
// });
...
// baza.put('/restoran/:uid', (req, res) => {
//     const { uid } = req.params;
//     const { Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend } = req.body;

//     const updateQuery = `
//         UPDATE Restoran
//         SET Naziv = ?, Kratak_Opis = ?, Email = ?, Lokacija = ?, Cena = ?, Datum_Osnivanja = ?, Sifra = ?, Sigurnosni_Kod = ?, RestoranPraviTortu = ?, Bend = ?
//         WHERE UID = ?
//     `;

//     conn.query(updateQuery, [Naziv, Kratak_Opis, Email, Lokacija, Cena, Datum_Osnivanja, Sifra, Sigurnosni_Kod, RestoranPraviTortu, Bend, uid], (err, result) => {
//         if (err) {
//             console.error(err);
//             return res.status(500).send('Internal Server Error');
//         }
...
//CRUD za Fotografa GOTOVO
//Ovo je poziv iz fronta
...
//         },
//         body: JSON.stringify({
//           NazivAgencija: values.nazivagencije,
//           Opis_Kompanije: values.opisKompanije,
//           Email: values.email,
//           Sigurnosni_Kod: values.sigurnosniKod,
//           Cena_Usluge: values.cenaUsluge,
//           Cena_Po_Slici: values.cenaposlici,
//           Datum_Osnivanja: values.datumosnivanja,
//           Lokacija: values.lokacija,
//           UID: auth.currentUser.uid,
//         }),
//       });
...
// baza.put('/fotograf/:id', (req, res) => {
//     const { id } = req.params;
//     const { NazivAgencije, Opis_Kompanije, Email, SigurnosniKod, Cena_Usluge, Cena_Po_Slici, Lokacija, Datum_Osnivanja, Broj_Telefona, SlobodniTermini } = req.body;
...
//         const updateQuery = `
//             UPDATE Fotograf
//             SET NazivAgencije = ?, Opis_Kompanije = ?, Email = ?, SigurnosniKod = ?, Cena_Usluge = ?, Cena_Po_Slici = ?, Lokacija = ?, Datum_Osnivanja = ?, Broj_Telefona = ?
//             WHERE ID = ?
//         `;

//         conn.query(updateQuery, [NazivAgencije, Opis_Kompanije, Email, SigurnosniKod, Cena_Usluge, Cena_Po_Slici, Lokacija, Datum_Osnivanja, Broj_Telefona, id], (err, result) => {
//             if (err) {
//                 console.error(err);
//                 return conn.rollback(() => {
//                     res.status(500).send('Internal Server Error');
...
//             const deleteTerminiQuery = 'DELETE FROM Slobodni_Termini WHERE Fotograf_ID = ?';
//             conn.query(deleteTerminiQuery, [id], (err) => {
//                 if (err) {
//                     console.error("Error deleting SlobodniTermini:", err);
//                     return conn.rollback(() => {
//                         res.status(500).send('Internal Server Error');
...
//                 if (SlobodniTermini && SlobodniTermini.length > 0) {
//                     const terminiValues = SlobodniTermini.map(termin => [termin, id]);
//                     const insertTerminiQuery = 'INSERT INTO Slobodni_Termini (Slobodan_Termin, Fotograf_ID) VALUES ?';
//                     conn.query(insertTerminiQuery, [terminiValues], (err) => {
//                         if (err) {
//                             console.error(err);
//                             return conn.rollback(() => {
...
//             conn.query(updateQuery, [NazivAgencije, Opis_Kompanije, Email, SigurnosniKod, Cena_Usluge, Cena_Po_Slici, Lokacija, Datum_Osnivanja, Broj_Telefona, uid], (err, result) => {
//                 if (err) {
//                     console.error(err);
//                     return conn.rollback(() => {
//                         res.status(500).send('Internal Server Error');
...
// baza.put('/fotograf/:id', (req, res) => {
//     const { id } = req.params;
//     const { NazivAgencije, Opis_Kompanije, Email, SigurnosniKod, Cena_Usluge, Cena_Po_Slici, Lokacija, Datum_Osnivanja, SlobodniTermini } = req.body;

//     const updateQuery = `
//         UPDATE Fotograf
//         SET NazivAgencije = ?, Opis_Kompanije = ?, Email = ?, SigurnosniKod = ?, Cena_Usluge = ?, Cena_Po_Slici = ?, Lokacija = ?, Datum_Osnivanja = ?
//         WHERE ID = ?
//     `;

//     conn.query(updateQuery, [NazivAgencije, Opis_Kompanije, Email, SigurnosniKod, Cena_Usluge, Cena_Po_Slici, Lokacija, Datum_Osnivanja, id], (err, result) => {
//         if (err) {
//             console.error(err);
//             return res.status(500).send('Internal Server Error');
//         }
...
// baza.delete('/fotograf/:uid', (req, res) => {
//     const { uid } = req.params;
//     var { slobodnitermini} = req.params.Slobodan_Termin.FotografID;

//     const deleteKorisnikQuery = `
//         DELETE FROM Korisnik
//         WHERE Fotograf_ID = (SELECT ID FROM Fotograf WHERE UID = ?)
//     `;
...
//         conn.query(deleteSlobodniTerminiQuery, [uid], (err, result) => {
//             if (err) {
//                 console.error(err);
//                 return res.status(500).send('Internal Server Error Slobodni_Termini');
//             }
...
//           Sifra: values.password,
//           Lokacija: values.lokacija,
//           Sigurnosni_Kod: values.sigkod,
//           Datum_Osnivanja: values.datum-osnivanja,
//           UID: auth.currentUser.uid,
//           SlobodniTermini: values.SlobodniTermini,
//         }),
//       });

//---------------------------------

// baza.post('/dekorater', upload.array('images', 10), (req, res) => {
//     const { Ime, Kratak_Opis, Email, Sifra, Lokacija, Sigurnosni_Kod, Datum_Osnivanja, Cena, Broj_Telefona, UID, SlobodniTermini } = req.body;
...
//             [Ime, Kratak_Opis, Email, Sifra, Lokacija, Sigurnosni_Kod, Datum_Osnivanja, Cena, Broj_Telefona, UID],
//             (err, result) => {
//                 if (err) {
//                     console.error(err);
//                     return conn.rollback(() => {
...
//                     .then(() => {
//                         if (SlobodniTermini && SlobodniTermini.length > 0) {
//                             const terminiValues = SlobodniTermini.map(termin => [termin, dekorater_id]);
//                             conn.query(
...
// baza.get('/dekorater', (req, res) => {
//     conn.query('SELECT * FROM Dekorater', (err, result) => {
//         if (err) throw new Error(err);
//         res.json(result);
//         // console.log(result);
//         // res.end();
...
//             Jelovnik: resultsJelovnik.find(item => item.Mladenci_ID === row.ID)
...
//           Prezime_Partnera: row.Prezime_Partnera,
//           Email: row.Email,
//           Fotograf_Termin: resultMladenci.find(item => item.Mladenci_ID === row.ID).Fotograf_Termin
//         }));
