    ...
    async function fetchProfileData() {
      try {
        const response = await axios.get(
          `http://localhost:5555/poslasticari/prekouid/${auth.currentUser.uid}`
        );
        ...
        const responseZakazano = await axios.get(
          `http://localhost:8080/zakazaniposlasticari/prekouid/${auth.currentUser.uid}`
        );
    ...
    if (change === true) {
      try {
        ...
        if (isValid) {
          ...
          await fetch(
            `http://localhost:8080/poslasticar/prekouid/${auth.currentUser.uid}`,
            {
              method: "PUT",
              headers: {
        ...
        if (selectedDate != null) {
          ...
          await fetch(
            `http://localhost:8080/slobodniterminiposlasticar/prekoid/${idPoslasticara}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
        ...
        const response = await axios.get(
          `http://localhost:8080/dekorater/prekouid/${auth.currentUser.uid}`
        );
        const fetchedData = response.data;
        setFreeDate(fetchedData.Slobodni_Termini);
        ...
        <div className=" flex  w-full flex-col rounded-md bg-snclblue p-5 text-white shadow-lg">
          ...
          <div>
            <div className="mb-3  flex flex-row flex-wrap items-center gap-3">
              <h1 className=" items-center text-xl font-extrabold">
                {t("Torte")}
              </h1>
              <input
                type="text"
                value={nazivTorte}
                onChange={(e) => setNazivTorte(e.target.value)}
                placeholder="Naziv torte"
                className="h-7 rounded-lg p-2 text-black"
              />
              <input
                type="number"
                value={cenaTorte}
                onChange={(e) => setCenaTorte(e.target.value)}
              ...
              <input
                type="text"
                value={opisTorte}
                onChange={(e) => setOpisTorte(e.target.value)}
            ...
            <div className="mb-3 flex flex-col gap-3">
              {torte.map((torta) => (
                ...
                >
                  <p>
                    <span className="font-bold">{torta.TIP}</span>
                  </p>
                  <p>{torta.Naziv}</p>
                  <p>
                    {torta.Cena} <span className="font-bold">din</span>
                  </p>
                  <p>{torta.Kratak_Opis}</p>
                  <button
                    type="button"
                    className="rounded-md bg-snclbrown p-1 text-black hover:bg-sncdbrown"
                    onClick={() => handleObrisi(torta.ID)}
                  >
                    Obrisi
                  </button>
                </div>
          ...
          <div className="   flex flex-row flex-wrap items-center justify-center">
            {zakazaniMladenci.map((zakazan) => (
              <div className="mx-2 mb-3 min-h-20 w-96 rounded-lg bg-snclbrown p-3 text-lg text-sncdblue shadow-lg">
                ...
                <p>
                  {t("datumSvadbe")}:{zakazan.Poslasticar_Termin}
                </p>
                <p>
                  {t("Cake")}
                  {zakazan.Naziv_Torte}
