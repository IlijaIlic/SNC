...
function OglasiPageDekorater() {
  ...
  const uniqueLocations = [
    ...new Set(ListaDekorater.map((dekorater) => dekorater._Lokacija)),
  ];
  ...
  const filterFotografiByDate = (dekorater) => {
    if (startDate && endDate) {
      return (
        dekorater._DatumOsnivanja >= startDate
        && dekorater._DatumOsnivanja <= endDate
      );
    }
    return true;
  };
  ...
  const filteredPosts = currentPosts.filter((dekorater) => {
    ...
    const isDateInRange = dekorater._DatumOsnivanja >= startDate
      && dekorater._DatumOsnivanja <= endDate;

    const isAgencyNameMatch = dekorater._NazivAgencije
      .toLowerCase()
      .includes(searchTerm.toLowerCase());

    const isDateWithinTimeframe = checkDateWithinTimeframe(
      dekorater._SlobodniTermini,
      selectedTimeframe,
    );
  ...
  return (
    <div className="flex h-full w-full flex-col">
      ...
      <div className=" flex h-1/6 w-full justify-between bg-snclbrown px-12 py-6 ">
        ...
        >
          <Link
            className="mx-2 flex items-center hover:text-snclbrown"
            to="/oglasi/fotograf"
          >
            {t('Fotograf')}
        ...
        <div className="m-4 flex h-full w-full flex-row flex-wrap ">
          {filteredPosts.map((dekorater, index) => (
            <div key={index} className="mb-8 ml-12 mr-2 h-2/6 w-3/12  min-w-96">
              <OglasiCardDekorater dekorater={dekorater} />
            </div>
          ))}
        </div>
      </div>
      <div className="mt-2 flex justify-center">
        {Array.from({
          length: Math.ceil(ListaDekorater.length / postsPerPage),
        }).map((_, index) => (
          <button
            key={index}
...
export default OglasiPageDekorater;
